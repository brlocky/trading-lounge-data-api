...
export class WaveInfoService {
  ...
  constructor() {
    ...
    this.patterns = [new MotiveExtended3(), new MotiveContractingDiagonal(), new MotiveExpandingDiagonal()];
  }
  ...
  getWaveInformation(wave1: Wave, wave2: Wave, wave3: Wave, wave4: Wave, wave5: Wave | null = null, useLogScale = true): WaveInfo[] {
    ...
    const getWave5Projection = (wave1: Wave, wave3: Wave, wave4: Wave, wave5: Wave): [number, number] => {
      const p0 = wave1.pStart.price;
      const p1 = wave1.pEnd.price;
      const p2 = wave3.pStart.price;
      const p3 = wave3.pEnd.price;

      const p4 = wave4.pEnd.price;
      const p5 = wave5.pEnd.price;

      this.fibonacci.setLogScale(useLogScale);
      const projectionWithWave1 = this.fibonacci.getProjectionPercentage(p0, p1, p4, p5);
      const projectionWithWave3 = this.fibonacci.getProjectionPercentage(p2, p3, p4, p5);

      return [projectionWithWave1, projectionWithWave3];
    };
    ...
    for (const p of this.patterns) {
      ...
      if (wave5) {
        const [projectionWave5WithWave1, projectionWave5WithWave3] = getWave5Projection(wave1, wave3, wave4, wave5);
        projectionWave5 = p.projectWave5WithWave1() ? projectionWave5WithWave1 : projectionWave5WithWave3;
        wave5Validation = p.validateWave5Projection(projectionWave5WithWave1, projectionWave5WithWave3);
      }
  ...
  getWaveInformation2(wave1: Wave, wave2: Wave, wave3: Wave, wave4: Wave, wave5: Wave | null = null, useLogScale = true): WaveInfo[] {
    const getWaveRetracement = (wave1: Wave, wave2: Wave): number => {
      const p1 = wave1.pStart.price;
      const p2 = wave1.pEnd.price;
      const p3 = wave2.pEnd.price;
